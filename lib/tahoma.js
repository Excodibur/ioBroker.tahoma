...
function Tahoma(username, password, context) {
	...
	this.lastRelogin = null;
	this.failedRequests = 0;
	this.retryLocked = false;
	
	this.deviceExecIds = {};
	
	this.ackStateValues = {};
	this.existingSlowStates = {};
}
 
Tahoma.prototype.unload = function(callback) {
	if(this.loginErrorTimeout) {
...
Tahoma.prototype.sendInternalRequest = function(method, requestPath, payload, callback) {
	...
	if(requestPath === 'login') {
		this.context.log.debug(method + " request on " + url + " with payload: +++redacted+++");
	} else {
...
Tahoma.prototype.sendInternalDELETE = function(requestPath, payload, callback) {
	return this.sendInternalRequest('DELETE', requestPath, payload, callback);
};
...
Tahoma.prototype.updateDevice = function(name, deviceData) {
	let features = this.getCreateStateOptions4Widget(deviceData.widget);
    ...
    this.createOrUpdateState(name, '', objset);
    ...
    for(var stateKey in deviceData.states) {
		...
		if(features.features && features.features.slow && features.features.slow === true) {
			...
			if(validStates.includes(state.name)) {
				this.existingSlowStates[name + '.states.' + state.name] = true;
				this.createOrUpdateState(name + '.states.' + state.name + ':slow', this.mapValueTahoma2ioBroker(state.name, state.value), this.getCreateStateOptions4State(deviceData.widget, state.name));	
			}
		}
    }
        ...
        if(command.nparams === 0) {
			...
			if(features.features && features.features.slow && features.features.slow === true) {
				...
				if(validCommands.includes(command.commandName)) {
					this.existingSlowStates[name + '.commands.' + command.commandName] = true;
					this.createOrUpdateState(name + '.commands.' + command.commandName + ':slow', false, {
						type: "boolean",
						read: true, 
						write: true,
						role: "button"
...
Tahoma.prototype.updateDeviceActionState = function(event, execId) {
	...
	if(!devicePath) {
		this.context.log.warn('Got action event for invalid device: ' + JSON.stringify(event));
		return;;
	}
...
Tahoma.prototype.updateDeviceState = function(event) {	
    ...
    for(var i in states) {
		...
		if('undefined' !== typeof this.existingSlowStates[key]) {
			this.context.setState(key + ':slow', value, true);
		}
    }
};
...
Tahoma.prototype.onDeploymentStateChange = function(id, value) {
    return this.onApplyChange('deployment', id, value);
};

Tahoma.prototype.onClosureStateChange = function(id, value, slow) {
    return this.onApplyChange('closure', id, value, slow);
};
