...
function Tahoma (username, password, url, loginOptions, applyOptions, context) {
    controller = this;
...
Tahoma.prototype.sendInternalRequest = function (method, requestPath, payload, callback) {
    let url = controller.url + requestPath;
    ...
    reqFunc({
        ...
        if (!error && response && (response.statusCode === 200 || response.statusCode === 204)) {
            ...
            controller.context.log.warn("error during tahomalink request: " + response.statusMessage + " ->" + response.statusCode + " retry " + requestPath);
            ...
            if (controller.loginAttempts >= controller.loginOptions.maxAttempts) {
                ...
                controller.context.log.info("Login failed three times, waiting " + controller.loginOptions.delayAfterFailure + " seconds before retrying.");
...
Tahoma.prototype.updateDevice = function (name, deviceData) {
    const features = controller.getCreateStateOptions4Widget(deviceData.widget);
    ...
    for (const commandKey in deviceData.definition.commands) {
        ...
        // nparams for commands could indicate how may parameters a command expects, but also seem to indicate how much impact a command can have:
        // 0: For IO devices standard device control commands (like open/close blinds)
        // 1: Commands that alter IO device configuration (e.g. pair, change device name). Could break user setup, if not used with care.
        //    However for RTS devices 1 still seems to be used for regular device control commands
        // 2: Seems to be used only for IO devices. Has major config impact, like reset to manufacturer settings.
        //
        // For now commands for IO devices are limited to classification nparams: 0, as other commands are to complex to bind them to states
        // and could even break device-configuration. As basic commands for RTS devices (for some unknown reason) are also sometimes classified
        // with nparams: 1, they are supported here.
...
Tahoma.prototype.updateDeviceActionState = function (event, execId) {
    ...
    const command = (event.command ? event.command : event.name);
