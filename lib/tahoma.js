...
Tahoma.prototype.sendInternalRequest = function (method, requestPath, payload, callback, locally) {
    ...
    if (!locally && requestPath.endsWith('apply') && payload && payload.actions && payload.actions.length === 1) {
        // only on POST
    ...
    if (!locally || method === 'POST') {
        if (jsonPayload !== null) {
            requestParams.json = jsonPayload;
        }
        if (formPayload !== null) {
            requestParams.form = formPayload;
        }
    }
        ...
        function (error, response, body) {
            if (!error && response && (response.statusCode === 200 || response.statusCode === 204)) {
                ...
                controller.context.log.warn(
                    `error during request: ${response.statusMessage} ->${response.statusCode} retry ${requestPath}`,
                );

                // session expired?
                if (locally) {
                    controller.clearBearerToken();
                }
                ...
                if (controller.loginAttempts >= controller.loginOptions.maxAttempts) {
                    ...
                    controller.context.log.info(
                        `Login failed ${controller.loginOptions.maxAttempts} times, waiting ${
                            controller.loginOptions.delayAfterFailure
                    ...
                    controller.context.log.debug(
                        `Will attempt again to login in ${controller.loginOptions.delayAttempts} seconds.`,
                    );
                ...
                if (
                    ...
                    (requestPath !== 'events/register' && !requestPath.match(/events\/*.\/fetch/g))
                ) {
                    if (requestPath === 'login') {
                        controller.context.log.warn(
                            `error during request: ${error}, request path: ${requestPath} with payload: +++redacted+++`,
                        );
                    } else {
                        controller.context.log.warn(
                            `error during request: ${error}, request path: ${requestPath} with payload:${JSON.stringify(
                                payload,
                            )}`,
                        );
                    }
                ...
                // Only reregister event listener
                if (requestPath.match(/events\/*.\/fetch/g) && body?.error === 'No registered event listener.') {
                    controller.context.log.warn(
                        'Apparently the event-listener expired server side. Creating a new one...',
                    );
                        ...
                        controller.sendPOST(
                            ...
                            function (err, data) {
                                if (err && controller.use_local_api) {
                                    controller.context.log.debug(
                                        'events/register failed. falling back to setup/devices',
                                    );
                                    controller.getSetup(callback);
                                    return;
                                }
                                controller.eventRegisterID = data.id;
                                controller.context.log.info(`New eventRegisterID = ${controller.eventRegisterID}`);
                                // controller.fetchEvents(callback);
                                controller.sendInternalRequest(method, requestPath, payload, callback, locally);
                            },
                            controller.use_local_api,
                        );
                    }, 10000);
                } else if (
                    controller.failedRequests > 5 ||
                    (body && body.errorCode && body.errorCode === 'UNSPECIFIED_ERROR')
                ) {
                    let delay;
...
Tahoma.prototype.clearBearerToken = function () {
...
Tahoma.prototype.login = function (callback) {
    ...
    controller.sendInternalPOST('login', payload, function (err, data) {
        ...
        if (controller.use_local_api) {
            ...
            // first get token
            controller.sendInternalGET(
                `config/${controller.context.config.gatewaypin}/local/tokens/generate`,
                {},
                function (err, data) {
                    ...
                    if (!err && data.token) {
                        ...
                        controller.sendInternalPOST(
                            `config/${controller.context.config.gatewaypin}/local/tokens`,
                            tokendata,
                            function (err, data) {
                                if (!err) {
                                    ...
                                    newDefaults.headers['Content-type'] = 'application/json';
                                    controller.localRequest = request.defaults(newDefaults);
                                    controller.context.getForeignObject(
                                        `system.adapter.${controller.context.namespace}`,
                                        (err, obj) => {
                                            if (err) {
                                                controller.context.log.error(
                                                    `Error while getting bearer token: ${err}`,
                                                );
                                                callback(err, data);
                                            }
                                            if (obj) {
                                                obj.native.bearer_token = bearerToken;
                                                controller.context.setForeignObject(
                                                    `system.adapter.${controller.context.namespace}`,
                                                    obj,
                                                );
                                            }
...
Tahoma.prototype.updateDevice = function (name, deviceData) {
    ...
    for (const stateKey in deviceData.states) {
        ...
        if (typeof state.value === 'undefined') {
            controller.context.log.debug(
                `State ${name}.states.${state.name} is undefined. Tahoma has no use for it, hence ignoring it.`,
            );
            ...
            controller.createOrUpdateState(
                `${name}.states.${state.name}`,
                controller.mapValueTahoma2ioBroker(state.name, state.value),
                controller.getCreateStateOptions4State(deviceData.widget, state.name),
            );

            if (features.features && features.features.slow && features.features.slow === true) {
                const validStates = ['core:ClosureState', 'core:TargetClosureState'];
                if (validStates.includes(state.name)) {
                    controller.existingSlowStates[`${name}.states.${state.name}`] = true;
                    controller.createOrUpdateState(
                        `${name}.states.${state.name}:slow`,
                        controller.mapValueTahoma2ioBroker(state.name, state.value),
                        controller.getCreateStateOptions4State(deviceData.widget, state.name),
                    );
                }
    ...
    for (const commandKey in deviceData.definition.commands) {
        ...
        if (
            (deviceData.deviceURL.startsWith('io:') && command.nparams === 0) ||
            ...
            if (features.features && features.features.slow && features.features.slow === true) {
                const validCommands = ['up', 'open', 'down', 'close'];
...
Tahoma.prototype.createOrUpdateChannel = function (key, states) {
    ...
    controller.context.setObjectNotExists(
        key,
        {
            type: 'channel',
            common: {
        ...
        () => {
            states.forEach(state => {
                ...
                controller.createOrUpdateState(
                    `${key}.${state.name}`,
                    controller.mapValueTahoma2ioBroker(state.name, state.value),
                    controller.getCreateStateOptions4State(null, state.name),
                );
            });
        },
    );
                    ...
                    for (let i = 0; i < objKeys.length; i++) {
                        ...
                        if (objid.match(search)) {
                            ...
                            controller.context.getState(objid, function (err, state) {
                                if (!err && state) {
                                    controller.Map_DeviceURL2StateName[state.val] = objid.substr(
                                        0,
                                        objid.indexOf('.deviceURL'),
                                    );
                                }
...
Tahoma.prototype.refresh = function (callback) {
    // controller.one is overridden below?!
    if (controller.use_local_api) {
        ...
        controller.sendPOST('setup/devices/states/refresh', null, function (err, data) {
            if (err) {
                controller.context.log.warn(
                    `refresh device state failed: Error - ${err} data: ${JSON.stringify(data)}`,
                );
            }
...
Tahoma.prototype.getAllStates = function (callback) {
    ...
    controller.login(function (err, _data) {
        if (err) {
            return;
        }
...
Tahoma.prototype.fetchEvents = function (callback) {
    ...
    controller.sendPOST(
        `events/${controller.eventRegisterID}/fetch`,
        {},
        function (err, data) {
            if (err) {
                return;
            ...
            controller.context.log.debug(
                `events/${controller.eventRegisterID}/fetch` + `Fetched Data ${JSON.stringify(data)}`,
            );
...
Tahoma.prototype.updateEventState = function (event) {
    ...
    try {
        ...
        if (currentState === 'COMPLETED' || currentState === 'FAILED') {
            ...
            controller.context.log.debug(
                `Skipping calling history. Using stored event (${JSON.stringify(storedEvent)}) instead`,
            );
            ...
            if (storedEvent === undefined) {
                controller.context.log.debug(
                    `Could not find event for execId ${event.execId} locally. Ignoring it for now.`,
                );
                return;
            }
                    ...
                    if (currentState === 'IN_PROGRESS' || currentState === 'FAILED') {
                        ...
                        controller.context.getState(commandStateId, (err, tmpState) => {
                            if (err || tmpState === null) {
                                controller.context.log.debug(
                                    `Command ${commandStateId} does not seem to be a changable state. Skipping it.`,
                                );
                            ...
                            switch (currentState) {
                                case 'IN_PROGRESS':
                                    controller.context.log.debug(
                                        `Command "${commandStateId}" ${currentState}. ACKnowledging it.`,
                                    );
                                    controller.context.setState(commandStateId, true, true);
                                    break;
                                case 'FAILED':
                                    controller.context.log.debug(
                                        `Command "${commandStateId}" ${currentState}. Setting it to false.`,
                                    );
        ...
        controller.context.log.warn(
            `Failure updating state for execId ${execId}. Probably no event was found. Root cause: ${exception}`,
        );
    }
};
...
Tahoma.prototype.updateDeviceActionState = function (event, execId) {
    ...
    if (undefined !== controller.ackStateValues[key]) {
        ...
        controller.context.log.debug(
            `${key} not found in ackStateValues. Values are: ${JSON.stringify(controller.ackStateValues)}`,
        );
        ...
        controller.context.getObject(key, (error, object) => {
            ...
            if (object.type === 'channel') {
                try {
                    ...
                    controller.context.log.warn(
                        `Could not state-data for channel ${key} Raw-data: ${value}. Exception: ${e}`,
                    );
                }
            } else {
                controller.ackStateValues[key] = value;
...
Tahoma.prototype.listHasDevice = function (_device, _list) {};
    ...
    controller.sendPOST(
        ...
        function (err, data) {
            if (
                ...
                (data.responseStatusCode === 400 || data.responseStatusCode === 503)
            ) {
                ...
                if (data.execId) {
                    controller.updateEventState({
                        execId: data.execId,
                        newState: 'IN_PROGRESS',
                        issuer: 'onClosureStateChange',
                    });
                }
            }
