...
Tahoma.prototype.updateDeviceActionState = function(event) {
	...
	let currentPos = null;
	let needNext = false;
	this.context.getState(devicePath + '.states.core:ClosureState', function(err, state) {
		if(err || !state || state.val === null) {
			this.context.getState(devicePath + '.states.core:DeploymentState', function(err, state) {
				if(err || !state || state.val === null) {
					this.context.getState(devicePath + '.states.core:OpenCloseState', function(err, state) {
						if(err || !state || state.val === null) {
							controller.context.log.info('Could not get direction of current movement.');
							return;
						} else {
							direction = 3;
							if(state.val === "open") {
...
Tahoma.prototype.onClosureStateChange = function(id, value, useDeployment, secondtry) {
    let controller = this;

	let commandName = "setClosure";
	let stateName = "core:ClosureState";
	if(useDeployment) {
	...
	controller.context.getState(id.substr(0, id.indexOf(".states.")) + ".deviceURL", function(err, state) {
		if(!err && state) {
			...
			controller.sendPOST("exec/apply", payload, function(err, data) {
				  if(err && data && data.responseStatusCode && data.responseStatusCode == 400) {
					  // sometimes a request is not accepted, so we try it again if we did not already
					  if(!secondtry) {
						  setTimeout(function() {
							  controller.onClosureStateChange(id, value, useDeployment, true);
						  }, 5000);
					  }
				  } else {
					  if(data.execId) {
...
Tahoma.prototype.onSetOrientation = function(id, value, secondtry) {
	let controller = this;
		...
		if(!err && state) {
			...
			controller.sendPOST("exec/apply", payload, function(err, data) {
				if(err && data && data.responseStatusCode && data.responseStatusCode == 400) {
					...
					if(!secondtry) {
						setTimeout(function() {
							controller.onSetOrientation(id, value, true);
						}, 5000);
					}
				} else {
					if(data.execId) {
						controller.updateEventState({execId: data.execId, newState: 'IN_PROGRESS', issuer: 'onSetOrientation'});
					}
				}
	        });	
		}
