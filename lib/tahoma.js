...
Tahoma.prototype.sendInternalRequest = function (method, requestPath, payload, callback, locally) {
    ...
    reqFunc(requestParams, function (error, response, body) {
        if (!error && response && (response.statusCode === 200 || response.statusCode === 204)) {
            ...
            if (!controller.use_local_api || (requestPath !== "events/register" && !requestPath.match(/events\/*.\/fetch/g))) {
                if (requestPath === "login")
            ...
            if (requestPath.match(/events\/*.\/fetch/g) && body?.error === "No registered event listener.") {
                controller.context.log.warn("Apparently the event-listener expired server side. Creating a new one...");
            ...
            } else if (controller.failedRequests > 5 || (body && body.errorCode && body.errorCode === "UNSPECIFIED_ERROR")) {
                let delay;
