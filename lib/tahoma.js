...
function Tahoma (username, password, url, local_url, loginOptions, applyOptions, context) {
    controller = this;

    controller.username = username;
    controller.password = password;
    controller.url = url;
	controller.local_url = local_url;
...
Tahoma.prototype.login = function (callback) {
	...
	if(controller.context.config.bearer_token) {
		...
		let new_defaults = controller.req_defaults;
		new_defaults.headers['Authorization'] = 'Bearer ' + controller.context.config.bearer_token;
		new_defaults.headers['Content-type'] = 'application/json';
		controller.localRequest = request.defaults(new_defaults);
			...
			controller.sendInternalGET("config/" + controller.context.config.gatewaypin + "/local/tokens/generate", {}, function(err, data) {
				if(!err) {
					...
					controller.sendInternalPOST("config/" + controller.context.config.gatewaypin + "/local/tokens", tokendata, function(err, data) {
						if(!err) {
							controller.context.config.bearer_token = bearer_token;
							let new_defaults = controller.req_defaults;
							new_defaults.headers['Authorization'] = 'Bearer ' + bearer_token;
							new_defaults.headers['Content-type'] = 'application/json';
							controller.localRequest = request.defaults(new_defaults);
							controller.context.getForeignObject('system.adapter.' + controller.context.namespace, (err, obj) => {
								if(obj) {
									obj.native.bearer_token = bearer_token;
					...
					controller.setConnected(false);
			        controller.loginInProgress = false;
					controller.context.log.err('Error getting token for local api');
					callback(err, data);
				}
			});
		} else {
...
Tahoma.prototype.updateGateWays = function (nextEntry) {
	...
	let use_url = "setup/gateways";
	if(!controller.use_local_api) {
		use_url = use_url + "/" + controller.gatewayList[nextEntry];
	}
    controller.sendGET(use_url, {}, function (err, data) {
        if (!err && data) {
...
Tahoma.prototype.getAllStates = function (callback) {
    ...
    controller.login(function (err, data) {
        ...
        if (controller.eventRegisterID === "-1") {
			// currently the event registration does not work on local api (UNKOWN OBJECT)
			if(!controller.use_local_api) {
