...
Tahoma.prototype.updateDevice = function(name, deviceData) {
    ...
    for(var commandKey in deviceData.definition.commands) {
        ...
        if(command.nparams === 0) {
            createOrUpdateState(name + '.commands.' + command.commandName, false, {
                read: true, 
                write: true,
                role: "button"
            });
...
Tahoma.prototype.createOrUpdateState = function(key, value, options) {
    ...
    this.context.getState(key, function(err, state) {
		if(err || !state || state.notExist) {
			...
			// create state
			controller.context.createState(key, value, 0, options);
		} else {
    ...
    this.sendGET('setup', {}, function(err, data) {
        if(!err) {
			...
			controller.context.getAdapterObjects(function(res) {
				...
				const search = new RegExp('^' + controller.context.namespace + '\.devices\.\d+\.deviceURL$');
				for(let i = 0; i < objKeys.length; i++) {
					let objid = objKeys[i];
					if(objid.match(search)) {
						controller.getState(objid, function(err, state) {
							if(!err && state) {
...
Tahoma.prototype.fetchEvents = function(callback) {
    ...
    this.sendPOST("events/" + eventRegisterID + "/fetch", {}, function (err,data) {
        if (err) {
            return;
        }
        
        controller.context.log.debug("events/" + eventRegisterID + "/fetch" + "Fetched Data" + data);
...
Tahoma.prototype.deleteOldDevices = function() {
	...
	this.context.getAdapterObjects(function(res) {
		const objKeys = Object.keys(res);
		const search = new RegExp('^' + controller.context.namespace + '\.devices\.\d+\.lastUpdateTime$');
		let delObjects = [];
